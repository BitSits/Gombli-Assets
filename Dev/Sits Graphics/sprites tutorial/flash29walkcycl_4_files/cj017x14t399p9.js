...
if(zRf==''){zRf=(zR.indexOf(uy)>-1)?0:(zR==''?-2:-1);for(var i=0;i<zRA.length;)if(zR.indexOf(zRA[i++]+'.')>-1)zRf=i;if(this.zRQO&&zRQ.length){i=zRQ.indexOf('&q=');if(i<0)i=zRQ.indexOf('?q=');if(i<0)i=zRQ.indexOf('?p=');if(i>0){zRQ=zRQ.substring(i+3);i=zRQ.indexOf('&');if(i>0)zRQ=zRQ.substring(0,i);zKW=es(zRQ,'kw=')}}}
zc(5,'zRf',zRf,30,'');
...
google_ad_output='js'
google_max_num_ads=isNaN(this.zGL)?9:zGL
...
mB=this.zD336&&(g==336)?3:1;ax='/'+bp+((td==uy)&&c.length?'abt.'+c:td)+'/'+c+'_'+gs+';'+zap+'svc='+s+';site='+(zgs.length?zgs:gs)+';t='+zTa+';bt='+zBT+';bts='+zBTS+zST(2,';st=')+((zDot==3)&&zbDM?zDMD():'')+';pc='+zpc+';auc='+(++zAUC)+';fd='+zFDT+';fs='+zFST+(this.zir?';r='+zir:'')+';sp2='+(zbXP2?1:0)+(zs>0?';go='+zGSk:'')+';a='+zc(0,'aut=')+tm+';kw='+k+';chan='+c+';syn=about;tile='+p+(mB==3?';af=0':'')+';r='+zRf+zTc()+(bT?';dcopt=ist':'')+';sz='+d+';dc_ref='+escape(zWl)+';ord='+(bH?'1':o)+zOr
...
if(1==zr(100))zau(256,1,1,'px3',zh+'z.'+uy+'/px/?p=ggl'+zs+(zG&16?'p':'b')+zTa+'_'+(this.zLb?zLb:0)+(this.zGCID?'&ggt='+zE(zGCID):'')+'&w='+zIfw+'&gs='+gs+'&tt='+zTa+'&bt='+zBT+'&bts='+zBTS+zST(2,'&st=')+'&bbaf='+z+'&a='+(zAC?1:0)+zSm+(zSu.length?'&su='+zSu:''),'')}
...
function zPxT(q){zPxA[zPxC]=new Image();zPxA[zPxC++].src=zh+'z.'+uy+'/px/?'+q}
...
while((i<l)&&i<zRad){t=a[i++].line1;w('<a href="/z/js/o'+(p?p:'')+'.htm?k='+escape(t.toLowerCase())+(this.zobr?zobr:'')+'&d='+escape(t)+'&r='+escape(zWl)+'" target="_'+(this.zOBNW?'new'+zr(9999):'top')+'">'+t+'</a>')}
...
function zSB(n,c){a=zAds;var i=zSbL,l=a.length;if(i>=l)return;if(!c)c=l;l=(i+c>l)?l:i+c;w(x5+'gB'+n+'"'+qf+'"gB"><h5>'+um+'s</h5>')
...
return t}
function Tac(g,j){if(zbM&&zbI)return;j=parseFloat(j/10);var v,i=0,f=new Image(),t="?"+Math.random()+"&snippet_version="+j+"&referrer="+zE(zRef)+"&page="+zE(zwl.href)+"&timezone="+(new Date()).getTimezoneOffset()
for(;i<tCL.length;i++){v=zc(4,tCL[i]);if(v!="")t+="&clist_"+zE(tCL[i])+"="+v}
t+=tCV();f.src=g+t}
function zTc(){var s=zc(6,'TID'),r=';u='+s+'|'+zTa;s=zc(6,'TData')
